...
class TopKNeighbors:
    ...
    def neighbors(self, idx: int) -> List[Tuple[int, float]]:
        ...
        if idx < 0 or idx >= self.n:
            raise ValueError(
                f"Index {idx} is out of bounds (valid range: 0 <= idx < {self.n})."
            )
    ...
    def distance_if_present(self, i: int, j: int) -> float | None:
        ...
        Args:
            ...
            j: Index of the neighbor to check (must be in range [0, n)).
        Returns:
        ...
        if j < 0 or j >= self.n:
            raise ValueError(
                f"Index j={j} is out of bounds (valid range: 0 <= j < {self.n})."
            )
        row = self.indices[i]
