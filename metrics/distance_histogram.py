...
from metrics.similarity_samples import PairSampleSpec, write_similarity_samples
...
class DistanceHistogramRunner:
    ...
    def run(
        ...
        *,
        ...
        sample_spec: PairSampleSpec | None = None,
    ) -> Path:
        ...
        Args:
            ...
            sample_spec: Optional settings for saving representative image-pair samples.
        Returns:
        ...
        for series_name, idxs in tqdm(series_to_indices.items()):
            series_set = set(idxs)
            if len(series_set) < 2:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Save representative image-pair samples grouped by percentile bins.",
    )
    parser.add_argument(
        "--samples-per-bin",
        type=int,
        default=3,
        help="Maximum number of image pairs to save per percentile bin when sampling is enabled.",
    )
    parser.add_argument(
        "--num-percentile-bins",
        type=int,
        default=10,
        help="Number of percentile bins to split each similarity pool into when sampling is enabled.",
    )
...
def main() -> None:
    ...
    sample_spec = (
        PairSampleSpec(
            ...
            num_percentile_bins=args.num_percentile_bins,
        )
        if args.save_similarity_samples
        else None
    )
    runner.run(
